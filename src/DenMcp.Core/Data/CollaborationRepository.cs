...
public sealed class CollaborationRepository : ICollaborationRepository
{
    ...
    private static class MarkdownBlockSegmenter
    {
        public static List<CollaborationSegment> Segment(string markdown, string segmenterVersion)
        {
            ...
            while (i < lines.Length)
            {
                ...
                else if (IsListItem(trimmedStart))
                {
                    ...
                    i++;
                    while (i < lines.Length && !string.IsNullOrWhiteSpace(lines[i]) && (IsListItem(lines[i].TrimStart()) || char.IsWhiteSpace(lines[i][0])))
                        i++;
                }
                else
                {
                ...
                segments.Add(new CollaborationSegment
                {
                    ...
                    SegmentType = type,
                    SegmentHash = HashText($"{segmenterVersion}\n{sequence}\n{type.ToDbValue()}\n{raw}"),
                    RawMarkdown = raw,
