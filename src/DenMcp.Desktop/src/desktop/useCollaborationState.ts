...
import {
  ...
  DenCollaborationSession,
  DenCollaborationSessionStatus,
  DenCollaborationTurn,
  type DenCollaborationApi,
} from './denCollaborationApi';

/** Client-side compiled annotation response format, matching CollaborationResponseCompiler semantics. */
function buildSnippet(segment: DenCollaborationSegment): string {
...
function formatAnnotationLine(annotation: DenCollaborationAnnotation): string {
...
export function compileResponse(
  ...
  const bySegmentId = new Map<number, DenCollaborationAnnotation[]>();
  ...
  for (const segment of segments) {
    ...
    const hashPrefix = segment.segment_hash.length >= 8 ? segment.segment_hash.slice(0, 8) : segment.segment_hash;
